...
## Auto-Order API - 來源網域功能

### 新增功能
1. **資料庫欄位**：`oms_order.source_domain VARCHAR(200)`
2. **自動偵測**：從 HTTP 請求頭取得來源網域
   - 優先順序：`sourceDomain 參數` > `Origin` > `Referer` > `X-Forwarded-Host` > `Remote Host`
3. **Model 更新**：`OmsOrder.sourceDomain` 欄位及 getter/setter
4. **Mapper 更新**：`OmsOrderMapper.xml` 所有 CRUD 操作
...
```bash
# 測試 Origin 自動偵測
curl -X POST http://localhost:8085/order/auto-generate \
  -H "Origin: https://test-casino.example.com" \
  -d '{"type": "collect", "amount": 1000, "name": "測試用戶", "phone": "0912345678"}'
# 結果：source_domain = test-casino.example.com ✅

# 測試手動傳入（優先級最高）
curl -X POST http://localhost:8085/order/auto-generate \
  -H "Origin: https://should-be-ignored.com" \
  -d '{"type": "pay", "amount": 500, "name": "代付測試", "phone": "0987654321", "sourceDomain": "manual-domain.tw"}'
# 結果：source_domain = manual-domain.tw ✅
```
...
4. 若匹配 → 記錄 `source_ip` 和 `source_domain`（source name）
